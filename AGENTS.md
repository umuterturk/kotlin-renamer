...
Before using `kr`, check if it is available:
```bash
which kr
```
...
```bash
# macOS (Apple Silicon)
curl -L https://github.com/umuterturk/kotlin-renamer/releases/latest/download/kr_darwin_arm64.tar.gz | tar xz && sudo mv kr /usr/local/bin/

# macOS (Intel)
curl -L https://github.com/umuterturk/kotlin-renamer/releases/latest/download/kr_darwin_amd64.tar.gz | tar xz && sudo mv kr /usr/local/bin/

# Linux (amd64)
curl -L https://github.com/umuterturk/kotlin-renamer/releases/latest/download/kr_linux_amd64.tar.gz | tar xz && sudo mv kr /usr/local/bin/
```
...
1. **Check `which kr` before use** — install if missing (see above).
2. **Prefer `--project` over `--file`** — catches all call sites.
3. **Use `--file` for `parameter`** — parameters are single-file scope.
4. **Use `--class` to narrow** when two classes share a method/property name.
5. **Dry-run first**: append `--dry-run`, review, then run without it.
6. **`kr move` always requires `--project`** — needs to scan all imports.
7. **Don't mix `kr` with `sed`/`str_replace`** on the same symbol.
